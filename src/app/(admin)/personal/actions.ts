...
export async function procesarCambioTurnoAction(
  ...
  // Use service-role client for all DB operations in this admin import action.
  // Admin users have empresa_id=null, so RLS with get_my_empresa_id() would block
  // all operario reads/writes. Server-side admin actions correctly bypass RLS.
  ...
  const { data: empresas, error: empresasError } = await adminDb
    .from('empresas')
  ...
  for (const [companyKey, workers] of groupedWorkers.entries()) {
    ...
    const { data: rpcData, error: rpcError } = await adminDb.rpc('procesar_cambio_turno', {
    ...
    for (const worker of workers) {
      ...
      // Usamos adminDb (service_role) para evitar que RLS bloquee la búsqueda
      const { data: matchedOp } = await adminDb
        .from('operarios')
        .select('id, activo')
      ...
      if (matchedOp) {
        ...
        const { error: updateError } = await adminDb.from('operarios').update({
        ...
        await adminDb.rpc('generar_creditos_checkin', {
  ...
  const { data: roomsForPreview, error: roomsPreviewError } = await adminDb
    .from('habitaciones')
    .select('id, bloque_id, capacidad, estado')

  const { data: activeWorkersByRoom, error: workersPreviewError } = await adminDb
    .from('operarios')
    .select('habitacion_id')
    .eq('activo', true)
...
export async function aplicarAsignacionesAction(
  ...
  const { data: roomsInfo } = await adminDb.from('habitaciones').select('id, capacidad').in('id', roomIdsToAssign)
  const { data: currentOcupants } = await adminDb.from('operarios').select('habitacion_id').in('habitacion_id', roomIdsToAssign).eq('activo', true)
  ...
  for (const assignment of parsed.data.assignments) {
    ...
    const { data: isAlreadyThere } = await adminDb.from('operarios').select('habitacion_id').eq('id', assignment.operario_id).single()
...
export async function intercambiarHabitacionesAction(
  ...
  // Get both operators
  const { data: op1 } = await adminDb.from('operarios').select('id, habitacion_id').eq('id', params.id1).single()
  const { data: op2 } = await adminDb.from('operarios').select('id, habitacion_id').eq('id', params.id2).single()
...
export async function crearOperarioManualAction(
  ...
  if (existing && !existing.activo) {
    // Re-activate
    const { error } = await adminDb.from('operarios').update({
      nombre_completo: parsed.data.nombre_completo,
      empresa_id: parsed.data.empresa_id,
      cargo: parsed.data.cargo,
  ...
  // Insert new
  const { data: newOp, error: insertError } = await adminDb.from('operarios').insert({
    nombre_completo: parsed.data.nombre_completo,
    documento_identidad: parsed.data.documento_identidad,
    empresa_id: parsed.data.empresa_id,
    cargo: parsed.data.cargo,
